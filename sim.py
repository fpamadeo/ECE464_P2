...
def main():
    ...
    # with concurrent.futures.ProcessPoolExecutor() as executor:
        # counterBin = executor.map(counterGen, [seed1 for seed1 in range(1, 2)]) #has all counters
        # lfsrSeqBin = executor.map(lfsrGen, [seed2 for seed2 in range(1, 3)])

    # print(*lfsrSeqBin, sep='\n')
    # input()
    ...
    all_data = open(os.path.join(script_dir, "Full_data.txt"), "w")
    all_data.write("Batchsize: " + repr(batchSize) + "\n")
